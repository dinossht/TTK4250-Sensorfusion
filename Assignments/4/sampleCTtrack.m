function [X, Z, a] = sampleCTtrack(n, Ts, xbar0, P0, q, r, PD, lambda)
    ...
    for k = 1:n
        ...
        if rand(1) < PD
            a(k) = randi(phi_k + 1, 1);
            Z{k} = [Zfalse(:, 1:(a(k) - 1)), ZX(:, k), Zfalse(:, a(k):end)];
        else
            Z{k} = Zfalse;
        end
    end
